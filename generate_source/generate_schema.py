...
grid_list_props = [(f"grid_list_{size}", boolean_type) for size in sizes]

spacing_props = [
    (f"{type_}{direction}_{size}", boolean_type)
    for type_ in ["m", "p"]
...
def kebab_to_camel(name):
    return "".join(map(lambda x: x.capitalize(), name.split("-")[1:]))
